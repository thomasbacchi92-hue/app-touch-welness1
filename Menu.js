/* Menu.js - Fix Anti-Doppione (iFrame Nativo App Mobile) */

window.injectMenu = function() {
    // 1. SISTEMA ANTI "FINESTRA NELLA FINESTRA"
    // Se questa pagina è caricata dentro l'App Mobile (tramite iframe), 
    // nascondiamo la navbar superiore e non carichiamo il menu desktop per evitare doppioni.
    if (window.self !== window.top) {
        ...
        return; // Blocca l'iniezione del menu desktop
    }

    // 2. LOGICA NORMALE PER IL DESKTOP
    if (document.getElementById('navMenu')) return; 

    const href = window.location.href.toLowerCase();
    const isSubFolder = href.includes('piscina') || href.includes('archivio');
    ...
    <div id="navMenu">
        ...
        <a href="${base}Statistiche.html" class="nav-link"><i class="material-icons-round">analytics</i> STATISTICHE STAFF</a>
